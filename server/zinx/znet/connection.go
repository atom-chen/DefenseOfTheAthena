...
func NewConnection(server ziface.IServer, conn *websocket.Conn, connId uint32, mh ziface.IMsgHandle) *Connection {
	...
	//将当前连接放入connMgr
	c.WsServer.GetConnectionMgr().Add(c)
...
func (c *Connection) StartReader() {
	...
	for {
		...
		log.Println("connection StartReader recv from client:", string(data))
		//得到request数据
		...
		req := NewRequest(c, temp.Cmd, temp.Msg)
		select {
		case <-c.exitChan:
			//读出错了
...
func (c *Connection) Stop() {
	...
	//将conn在connMgr中删除
	c.WsServer.GetConnectionMgr().Remove(c)
